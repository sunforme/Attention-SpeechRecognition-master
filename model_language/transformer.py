...
def multihead_attention(emb,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # Multiplication
        outputs = tf.matmul(Q_, tf.transpose(K_, [0, 2, 1]))  #将k进行转置 (h*N, T_q, T_k)
        
        # Scale
        outputs = outputs / (K_.get_shape().as_list()[-1] ** 0.5)
        
        # Key Masking
        key_masks = tf.sign(tf.abs(tf.reduce_sum(emb, axis=-1))) # (N, T_k) 计算张量tensor沿着某一维度的和，求绝对值，返回一个数学符号
        key_masks = tf.tile(key_masks, [num_heads, 1]) # (h*N, T_k)对张量(Tensor)进行扩展
